...
extern "C" void gb_mxm_like_partition_merge_nc(GrB_Matrix &C, GrB_Matrix &M,
    ...
    if (IC[nrows_C] == 0) {
        // New matrix
        info = GrB_Matrix_new(&C, GrB_UINT64, nrows_M, ncols_M);
    } else {
        std::vector<uint64_t> VC(JC.size(), 1);
        // Import C as CSR
        info = GrB_Matrix_import_UINT64(&C, GrB_UINT64, nrows_M, ncols_M, IC.data(),
                                      JC.data(), VC.data(), IC.size(), JC.size(),
