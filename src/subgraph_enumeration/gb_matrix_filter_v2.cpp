...
extern "C" void gb_matrix_filter_v2(GrB_Matrix &C, GrB_Matrix M, GrB_Matrix B,
    ...
    printf("(My MxM CN) Binaralization: ");
    simple_tic(tic);
    {
    ...
    simple_tic(tic);
    { gb_mxm_partition(C, M, A, B); }
    result = simple_toc(tic);
    printf("(My MxM CN) MxM: %f ms\n", result * 1e3);

    printf("(My MxM CN) Filter: ");
    simple_tic(tic);
    {
...
extern "C" void _gb_matrix_filter_v2(GrB_Matrix *C, GrB_Matrix *M,
