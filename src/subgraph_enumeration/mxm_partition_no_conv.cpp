...
extern "C" void mxv_v1(std::vector<size_t> &Ic, size_t *Im, size_t Im_size,
                       size_t *IA, size_t IA_size, size_t *JA, size_t JA_size,
                       size_t *Ib, size_t Ib_size);

extern "C" void mxm_partition_no_conv(
    size_t ***IC, size_t **IC_size, size_t ***JC, size_t **JC_size,
    size_t ***VC, size_t **VC_size, size_t *IM, size_t IM_size, size_t *JM,
    size_t JM_size, size_t *IB, size_t IB_size, size_t *JB, size_t JB_size,
...
#pragma omp parallel for num_threads(num_threads)
    for (size_t partition = 0; partition < num_threads; partition++) {
        for (size_t ib = partition_offset[partition];
            ...
            mxv_v1(tmp_C, M_sorted.data(), M_sorted.size(), IA_arr, IA_size,
                   JA_arr, JA_size, B_st, B_size);
